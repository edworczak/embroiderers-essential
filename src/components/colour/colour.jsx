...
const Colour = ({
    ...
    const substituteLine = (brand, substitute) => {
        return (
            substitute && <TableRow label={brand.name} text={substitute[0].replace(brand.code, "")} key={brand.code} />
        );
    };
            ...
            <CardCTAsWrapperStyled>
                {deleteCTA && (
                    <IconButton
                        ...
                        label={"usuń"}
                        iconOnly={true}
                        onClick={() => dispatch(deleteThread(colourID))}
                    />
                )}
                ...
                {collectionCTA && !isInStock && (
                    <IconButton icon={faBoxesStacked} label={"do zapasów"} iconOnly={true} onClick={addToStock} />
                )}
            </CardCTAsWrapperStyled>
        </Card>
    );
