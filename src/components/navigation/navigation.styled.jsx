...
export const ScreenOverlay = styled.div`
	background-color: ${(props) =>
		props.$expanded ? "rgb(0, 105, 137, 0.2)" : "rgb(0, 105, 137, 0)"};
	height: 100vh;
	left: 0;
	position: fixed;
	top: 0;
	...
	width: ${(props) =>
		props.$opening || props.$closing || props.$expanded ? "100%" : "50px"};
	z-index: 1;
`;
...
export const NavigationWrapper = styled.nav`
	...
	width: ${(props) =>
		props.$expanded ? "200px" : theme.sizes.spacings.navWidth};
	z-index: 2;
	...
	button {
		...
		span {
			...
			width: ${(props) =>
				props.$expanded && !props.$opening ? "calc(100% - 22px)" : "0"};
		}

		svg {
			margin-left: ${(props) =>
				props.$expanded && !props.$opening ? "16px" : "0"};
