                ...
                <ProjectHeader $border={theme.colours.white} $useFlex={true} $background={"white"}>
                    <Row lightBackground={true}>
                        ...
                        <TableBody colourRows={true}>
                            {project.startDate && (
                                <TableRow label={pl.project.start.default} text={project.startDate} />
                            )}
                            {project.finishDate && (
                                <TableRow label={pl.project.finish.default} text={project.finishDate} />
                            )}
                    ...
                    <ProjectImageWrapper $placeholderImage={!project.img}>
                        <img
                            src={project.img ? project.img : theme.decorations.defaultImages.project}
                                ...
                                project.budget.costs.map((cost) => {
                                    return (
                                        <TableRow
                                            key={cost.id}
                                            label={cost.label}
                                            text={`-${cost.cost} ${dummyUser.user.currency}`}
                                        />
                                    );
                                })}
                            {project.budget.price && (
                                <TableRow
                                    label={pl.project.budget.price}
                                    text={`${project.budget.price} ${dummyUser.user.currency}`}
                                />
                            )}
                            {project.budget && (
                                <TableRow
